...
def inference(file_path, out_dir):
    """Inference function """
    ...
    train_saver.restore(sess, './monodepth')
   
    disp = sess.run(model.disp_left_est[0])

    disparities_pp = np.zeros((params.height, params.width), dtype=np.float32)
    disparities_pp = post_process_disparity(disp.squeeze())

    plt.imshow(disparities_pp, cmap='gray', interpolation='nearest', vmin=0, vmax=1)
